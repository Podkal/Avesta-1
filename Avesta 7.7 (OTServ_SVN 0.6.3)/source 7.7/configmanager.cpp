...
bool ConfigManager::loadFile(const std::string& _filename)
{
	...
	m_confBool[CHANGESEX_ONLY_PREMIUM] = getGlobalBoolean(L, "changeSexOnlyPremium", true);
	m_confInteger[SERVER_SAVE] = getGlobalNumber(L, "ServerSave", 2000 /*Padrão: A cada 2000 minutos */);
	m_confInteger[PUSH_INTERVAL] = getGlobalNumber(L, "PushDelayInterval", 1000);
	m_confBool[SPELLS_ORANGE] = getGlobalBoolean(L, "spellTalkInOrange", false);
	m_confString[SERVER_PRIORITY] = getGlobalString(L, "serverPriority", "aboveNormal");
	m_confBool[FREE_PREMIUM] = getGlobalBoolean(L, "freePremium", false);
	m_confString[TYPE_FORMULA] = getGlobalString(L, "ExpFormulaType", "original");
	
	// Real Tibia Save System
	m_confBool[ENABLE_SERVER_SAVE] = getGlobalBoolean(L, "enableServerSave", false);
	m_confString[SERVER_SAVE_MODE] = getGlobalString(L, "serverSaveMode", "closeserver");
	m_confInteger[SERVER_SAVE_HOUR] = getGlobalNumber(L, "serverSaveHour", 7);
	m_confInteger[SAVE_WARN_BEFORE] = getGlobalNumber(L, "saveWarnBefore", 10);
	m_confBool[SAVE_PLAYERS] = getGlobalBoolean(L, "savePlayers", true);
	m_confBool[CLEAN_SERVER_ATSAVE] = getGlobalBoolean(L, "cleanServerAtSave", true);
	m_confInteger[SIMPLE_SAVE_FREQUENCE] = getGlobalNumber(L, "simpleSaveFrequence", 0);
    m_confInteger[AUTO_CLEAN_FREQUENCE] = getGlobalNumber(L, "autoCleanFrequence", 0);
    m_confInteger[CLEAN_WARN_BEFORE] = getGlobalNumber(L, "cleanWarnBefore", 0);
    m_confBool[SAVE_WARNING] = getGlobalBoolean(L, "saveWarning", false);
    m_confBool[CLEAN_PZ] = getGlobalBoolean(L, "cleanPz", false);
    m_confBool[CLEAN_PZ_SAVE] = getGlobalBoolean(L, "cleanPzAtSave", true);
    m_confBool[CLEAN_CORPSES_SAVE] = getGlobalBoolean(L, "cleanCorpsesAtSave", true);
    m_confBool[CLEAN_CORPSES] = getGlobalBoolean(L, "cleanCorpses", false);
    m_confBool[DATABASE_BACKUP_ATSAVE] = getGlobalBoolean(L, "DatabaseBackupAtSave", true);
