...
{
    if(premiumEnd > std::time(NULL))
        return true;
    else if(hasFlag(PlayerFlag_IsAlwaysPremium))
        return true;
    else if(g_config.getBool(ConfigManager::FREE_PREMIUM))
        return true;
    else
        return false;
}
...
void Player::dropLoot(Container* corpse)
{
    ...
    if (getSkull() != SKULL_RED){
        Item *_amulet = getInventoryItem(SLOT_NECKLACE);
        if (_amulet != NULL)
        {
            if (_amulet->getID() == 2173) {
                g_game.removeItemOfType(this, 2173, 1);
                return;
            }
        }
    } 	
    ...
    if (getSkull() != SKULL_RED){
        Item *_amulet = getEquippedItem(SLOT_NECKLACE);
        if (_amulet != NULL)
        {
            if (_amulet->getID() == 2173) {
                g_game.removeItemOfType(this, 2173, 1);
...
void Player::addUnjustifiedDead(const Player* attacked)
{
	...
	if(redSkullTicks >= g_config.getNumber(ConfigManager::KILLS_TO_BAN) * g_config.getNumber(ConfigManager::FRAG_TIME)){
	...
	else if(g_config.getNumber(ConfigManager::KILLS_TO_BAN) != 0 && redSkullTicks >= (g_config.getNumber(ConfigManager::KILLS_TO_BAN) - 1) * g_config.getNumber(ConfigManager::FRAG_TIME))
	{
		...
		if (success) {
			...
			Scheduler::getScheduler().addEvent(createSchedulerTask(1000, boost::bind(&Player::kickPlayer, this)));
		}
	}
