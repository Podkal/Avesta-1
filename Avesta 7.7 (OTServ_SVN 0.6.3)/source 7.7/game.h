...
public:
  Game();
	...
	void callSave();
	void saveGameState(bool saveMap, bool savePlayers, bool countFrequence);
	void cleanGameState(uint32_t warnBefore, bool countFrequence);
	uint32_t cleanMap();
	void beforeServerSave(uint32_t minutes);
	void serverSaveNow();
	void loadGameState();
