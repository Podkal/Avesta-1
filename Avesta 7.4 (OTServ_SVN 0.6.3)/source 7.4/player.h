...
public:
	...
	{
		level--;
		if(g_config.getBool(ConfigManager::TIBIANIC_FORMULA))
            return (level * 35ULL) * (level * level * level) / 500ULL + 10ULL * level; // Tibianic Formula
        else
            return ((50ULL * level * level * level) - (150ULL * level * level) + (400ULL * level))/3ULL; // Original Formula
	}*/
